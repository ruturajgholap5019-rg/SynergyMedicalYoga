...
export default function CheckoutPage({ cart, currentUser, onOrderComplete, setActivePage }) {
  ...
  const [paymentMethod, setPaymentMethod] = useState('cashfree'); // default 'cashfree'
  ...
  const handleSubmitOrder = async (e) => {
    ...
    try {
      ...
      // 1. Process Cashfree Payments Gateway (Primary)
      if (paymentMethod === 'cashfree' && response?.paymentSessionId) {
        toast.info('⏳ Opening Cashfree Secure Payment Gateway...', { autoClose: 2000 });
        ...
        if (CashfreeSDK) {
          ...
          toast.error('Unable to initialize Cashfree SDK. Please check your internet connection.');
        }
      }
    ...
    return (
      <div className="min-h-[70vh] bg-slate-50 flex items-center justify-center p-6">
        <div className="bg-white max-w-xl w-full rounded-3xl p-8 sm:p-12 shadow-xl border border-slate-100 text-center space-y-6 animate-in fade-in zoom-in-95 duration-300">
          ...
          <h2 className="text-2xl sm:text-3xl font-black text-slate-900">Thank You for Your Order!</h2>
          <p className="text-slate-600 text-sm sm:text-base leading-relaxed">
            Your payment and delivery details have been received by <strong className="text-[#005550]">Synergy Medical Yoga</strong>. Your order tracking reference is:
          </p>
          ...
          <p className="text-xs text-slate-400">
            A confirmation email has been sent to <span className="font-medium text-slate-700">{email}</span> with instructions and delivery updates.
          </p>
      ...
      <div className="max-w-[1240px] mx-auto px-4 sm:px-8 mt-10 sm:mt-12">
        <form onSubmit={handleSubmitOrder} className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
          ...
          <div className="lg:col-span-5 space-y-6 lg:sticky lg:top-28">
            ...
            {/* 3. Payment Gateways Card exactly replicating original website's Cashfree Payments UI */}
            <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-[0_6px_30px_-5px_rgba(0,0,0,0.07)] border border-slate-100/80 space-y-6">
                ...
                <div className="p-4 rounded-2xl border border-[#005550] bg-[#f8fbfa] shadow-sm transition-all select-none">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <span className="font-bold text-sm text-slate-800">Cashfree Payments</span>
                    </div>
                    
                    {/* Cashfree Badge simulating original website's icon */}
                    <div className="flex items-center gap-1 font-black text-[11px] tracking-tight px-2.5 py-1 bg-white rounded-lg border border-slate-200/80 shadow-3xs">
                      <span className="text-[#09b85c] font-black text-sm leading-none">C</span>
                      <span className="text-slate-700">Cashfree</span>
                      <span className="text-[#6842e4] font-extrabold">Payments</span>
                    </div>
                  </div>

                  {/* Cashfree Explanation Box exactly matching original photo */}
              ...
              <div className="pt-2">
                <p className="text-xs text-slate-500 leading-relaxed">
                  ...
                  <span onClick={() => toast("View privacy policy")} className="text-[#005550] underline font-medium cursor-pointer">
                    privacy policy
                  </span>.
                </p>
              </div>
