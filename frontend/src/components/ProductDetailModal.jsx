...
export default function ProductDetailModal({ product, onClose, onAddToCart, onBuyNow }) {
  ...
  const images = rawImages.map(img => getImageUrl(img) || '/favicon.svg');
            ...
            {images.length > 1 && (
              <div className="flex gap-3 overflow-x-auto pb-2">
                {images.map((img, idx) => (
                  ...
                  >
                    <img src={img} alt="" onError={(e) => { e.target.onerror = null; e.target.src = '/favicon.svg'; }} className="w-full h-full object-cover" />
                  </button>
                ))}
              </div>
            )}
